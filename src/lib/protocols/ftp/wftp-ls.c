...
static struct fileinfo *ftp_parse_unix_ls(const char *file,
                                          int ignore_perms)
{
// TODO: Remove this ifdef!
    ...
    while ((line = read_whole_line(fp)) != NULL) {
        ...
        /* While there are tokens on the line, parse them.  Next is the
           ...
           works for now.  */
        tok = line;
        while (ptok = tok, (tok = strtok(NULL, " ")) != NULL) {
            --next;
            if (next < 0) {     /* a month name was not encountered */
                ...
                if (year)
                    ...
                    DEBUGP(("time: %02d:%02d:%02d (no yr); ", hour, min,
                            sec));
                ...
                if (fnlen) {
                    if (cur.type == FT_DIRECTORY
                        && cur.name[fnlen - 1] == '/') {
                        cur.name[fnlen - 1] = '\0';
                        DEBUGP(("trailing `/' on dir.\n"));
                    } else if (cur.type == FT_SYMLINK
                               && cur.name[fnlen - 1] == '@') {
                        cur.name[fnlen - 1] = '\0';
        ...
        *date_str = '\0';
        ...
        while (tok != NULL) {
            ...
            if ((strlen(tok) < 12) && (strchr(tok, '-') != NULL)) {
                ...
                DEBUGP(("Time. "));
                strncat(date_str,
                        tok, (sizeof(date_str) - strlen(date_str) - 1));
                DEBUGP(("Date time: >%s<\n", date_str));
...
struct fileinfo *ftp_parse_ls(const char *file,
                              const enum stype system_type)
{
    switch (system_type) {
    case ST_UNIX:
