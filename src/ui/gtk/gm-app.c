...
GmBookManager *gm_app_peek_book_manager(GmApp * self)
{
...
{
    g_action_group_activate_action(G_ACTION_GROUP(self), "new-window",
                                   NULL);
}
...
{
    g_action_group_activate_action(G_ACTION_GROUP(self), "search",
...
about_cb(GSimpleAction * action, GVariant * parameter, gpointer user_data)
{
    ...
    gtk_show_about_dialog(NULL,
                          ...
                          (strcmp(translator_credits, "translator_credits")
                           != 0 ? translator_credits : NULL), "website",
                          "https://github.com/yangyingchao/mget",
                          "website-label", _("Gmget Website"),
                          "logo-icon-name", "gmget", NULL);
}
...
static void
search_cb(GSimpleAction * action, GVariant * parameter, gpointer user_data)
{
    GmApp *self = GM_APP(user_data);
    GtkWindow *window;
    const gchar *str;
...
static void setup_actions(GmApp * self)
{
...
static void setup_accelerators(GmApp * self)
{
    gtk_application_add_accelerator(GTK_APPLICATION(self), "<Primary>0",
                                    "win.zoom-default", NULL);
    gtk_application_add_accelerator(GTK_APPLICATION(self),
                                    "<Primary>minus", "win.zoom-out",
                                    NULL);
    gtk_application_add_accelerator(GTK_APPLICATION(self), "<Primary>plus",
                                    "win.zoom-in", NULL);
    gtk_application_add_accelerator(GTK_APPLICATION(self), "<Primary>k",
                                    "win.focus-search", NULL);
    gtk_application_add_accelerator(GTK_APPLICATION(self), "<Primary>f",
                                    "win.find", NULL);
    gtk_application_add_accelerator(GTK_APPLICATION(self), "<Primary>c",
                                    "win.copy", NULL);
    gtk_application_add_accelerator(GTK_APPLICATION(self), "<Primary>p",
                                    "win.print", NULL);
    gtk_application_add_accelerator(GTK_APPLICATION(self), "<Primary>t",
                                    "win.new-tab", NULL);
    gtk_application_add_accelerator(GTK_APPLICATION(self), "<Primary>w",
                                    "win.close", NULL);
    gtk_application_add_accelerator(GTK_APPLICATION(self), "F10",
                                    "win.gear-menu", NULL);
    gtk_application_add_accelerator(GTK_APPLICATION(self), "<Alt>Right",
                                    "win.go-forward", NULL);
    gtk_application_add_accelerator(GTK_APPLICATION(self), "<Alt>Left",
                                    "win.go-back", NULL);
}

/******************************************************************************/

static void setup_menu(GmApp * self)
{
    GMenuModel *model;
    GError *error = NULL;
    model =
...
GmApp *gm_app_new(void)
{
    ...
    gtk_window_set_default_icon_from_file
        ("/mnt/data/Work/mget/src/ui/gtk/res/gmget.png", NULL);
...
static void gm_app_init(GmApp * self)
{
    self->priv =
        G_TYPE_INSTANCE_GET_PRIVATE(self, GM_TYPE_APP, GmAppPrivate);
}
...
static void gm_app_class_init(GmAppClass * klass)
{
